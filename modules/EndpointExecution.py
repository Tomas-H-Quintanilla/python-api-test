...
def get_payload(endpoint_data,test_data):
    if 'payload_file' in endpoint_data and "payload" not in endpoint_data:
        endpoint_data['payload'] = replace_vars(load_file_data(f'{test_data["name"]}/data/{endpoint_data['payload_file']}'),test_data['variables'])
...
def execute_endpoint(endpoint, test_data):
    ...
    if not "url_file" in endpoint_data:
        ...
        urls= replace_vars(load_file_data(f'{test_data["name"]}/data/{endpoint_data['url_file']}'),test_data['variables'])
        for url in urls:
